...
namespace WaDEImportFunctions
{
    public class ImportReportingUnits
    {
        ...
        {
            return await Import.LoadBatch(batchData, BatchFunctionName, WaterAllocationManager.LoadReportingUnits, log);
        }

        [Function(CountFunctionName)]
        public async Task<int> GetCount([ActivityTrigger] string runId, ILogger log)
        {
            return await Import.GetCount(runId, CountFunctionName, WaterAllocationManager.GetReportingUnitsCount, log);
        }
    }
}
