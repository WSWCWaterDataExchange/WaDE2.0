...
namespace WaDEImportFunctions
{
    public class ImportSiteSpecificAmounts
    {
        ...
        {
            return await Import.LoadBatch(context, BatchFunctionName, WaterAllocationManager.LoadSiteSpecificAmounts, log);
        }

        [FunctionName(CountFunctionName)]
        public async Task<int> GetCount([ActivityTrigger] IDurableActivityContext context, ILogger log)
        {
            return await Import.GetCount(context, CountFunctionName, WaterAllocationManager.GetSiteSpecificAmountsCount, log);
        }
    }
}
