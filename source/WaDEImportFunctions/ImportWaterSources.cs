...
namespace WaDEImportFunctions
{
    public class ImportWaterSources
    {
        ...
        {
            return await Import.LoadBatch(context, BatchFunctionName, WaterAllocationManager.LoadWaterSources, log);
        }

        [FunctionName(CountFunctionName)]
        public async Task<int> GetCount([ActivityTrigger] IDurableActivityContext context, ILogger log)
        {
            return await Import.GetCount(context, CountFunctionName, WaterAllocationManager.GetWaterSourcesCount, log);
        }
    }
}
