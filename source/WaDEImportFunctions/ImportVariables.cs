...
namespace WaDEImportFunctions
{
    public class ImportVariables
    {
        ...
        {
            return await Import.LoadBatch(context, BatchFunctionName, WaterAllocationManager.LoadVariables, log);
        }

        [FunctionName(CountFunctionName)]
        public async Task<int> GetCount([ActivityTrigger] IDurableActivityContext context, ILogger log)
        {
            return await Import.GetCount(context, CountFunctionName, WaterAllocationManager.GetVariablesCount, log);
        }
    }
}
